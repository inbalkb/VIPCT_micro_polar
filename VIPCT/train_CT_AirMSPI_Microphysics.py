...
from VIPCT.microphysics_airmspi_dataset import get_microphysics_airmspi_datasets, trivial_collate
...
@hydra.main(config_path=CONFIG_DIR, config_name="microphysics_train_airmspi_sw")
def main(cfg: DictConfig):
    ...
    model = CTnetMicrophysicsAirMSPI(cfg=cfg, n_cam=cfg.data.n_cam)
    ...
    if stats is None:
        if cfg.optimizer.lwc_loss == 'L2_relative_error_w_score':
            stats = Stats(
    ...
    for epoch in range(start_epoch, cfg.optimizer.max_epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            elif cfg.optimizer.lwc_loss == 'L2_relative_error_w_score':
