...
class PerspectiveCameras(TensorProperties):
    ...
    def project_points_at3d(
        ...
        if screen:
            points_out = self._to_screen_transform(points_out)
        self.index = torch.arange(self._N)
        points_out_swap = [] # swap x,y coordinates according to the at3d input
        for uv in points_out:
            uv_swap = torch.zeros(uv.shape, device=points[0].device)
            uv_swap[..., 0] = uv[..., 1]
            uv_swap[..., 1] = uv[..., 0]
            points_out_swap.append(uv_swap)
        return points_out_swap
